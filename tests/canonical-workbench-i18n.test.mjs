...
test('canonical workbench persists and exposes all supported UI languages', () => {
  assert.match(workbench, /localStorage\.setItem\("poligome-language", next\)/);
  for (const language of ['pt', 'en', 'fr', 'es']) assert.match(workbench, new RegExp(`option value="${language}"`));
  assert.match(workbench, /aria-label=\{copy\.language\}/);
});
