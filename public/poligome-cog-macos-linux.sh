...
APP_DIR="${HOME}/.poligome-cog"
VENV_DIR="${APP_DIR}/venv"
CONNECTOR="${APP_DIR}/poligome-cog-local.py"
READY_FILE="${APP_DIR}/dependencies-v1.ok"
SAIDA_DIR="${APP_DIR}/convertidos"
SITE_URL="https://www.poligome.com/anotar"

printf '\n==========================================\n'
printf '       Poligome local COG\n'
printf '==========================================\n'
printf 'Converts a large GeoTIFF to COG on your own machine.\n'
printf 'Na primeira execucao, a preparacao pode demorar alguns minutos.\n\n'

command -v python3 >/dev/null 2>&1 || {
  printf 'Python 3 not found. Install python3 and run this file again.\n'
  exit 1
}

mkdir -p "${APP_DIR}" "${SAIDA_DIR}"

# The converter is embedded after the marker so the installer is a single file.
awk 'f{print} /^# === POLIGOME_PYTHON ===$/{f=1}' "$0" > "${CONNECTOR}"
...
if [ ! -f "${READY_FILE}" ]; then
  printf 'Installing rasterio, rio-cogeo and the local server. Please wait...\n'
...
exec "${PYTHON}" "${CONNECTOR}" --port 7861 --folder "${SAIDA_DIR}"
