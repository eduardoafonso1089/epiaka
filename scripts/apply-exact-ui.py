...
workbench = Path("app/editor/workbench/canonical-editor-workbench.tsx")
s = workbench.read_text()
if "PreRefactorTopbar" not in s:
    s = s.replace(
  ...
  return <main className="shell" aria-label={`${copy.appTitle}: ${projectName}`}>
    ...
    workbench.write_text(s)

panels = Path("app/editor/panels/editor-management-panels.tsx")
p = panels.read_text()
if "onLoadDemo?:" not in p:
    p = p.replace(
    ...
    p = p.replace(
        "className={classes(ui.panelCard, ui.imageRail)}>",
        'className={classes("assets", leftOpen && "open", ui.panelCard, ui.imageRail)}>',
    )
    p = p.replace(
        "className={ui.rightRail}>",
        'className={classes("labels", rightOpen && "open", ui.rightRail)}>',
    )
    anchor = '''      <div className={ui.panelHeader}>
    ...
    if anchor not in p:
        raise SystemExit("panel header anchor not found")
